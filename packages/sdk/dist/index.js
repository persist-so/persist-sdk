...
var PersistClient = class {
  ...
  async upload(filePath, options = {}) {
    ...
    if (targetSize > CHUNK_SIZE) {
      const startRes = await import_axios.default.post(`${this.apiUrl}/v1/upload/multipart/start`, { fileName }, { headers: this.headers });
      ...
      const presignRes = await import_axios.default.post(`${this.apiUrl}/v1/upload/multipart/presign`, {
        key,
        uploadId,
        parts: partsArr
      ...
      for (let i = 0; i < partsArr.length; i += maxConcurrency) {
        ...
        const promises = chunkBatch.map(async (partNumber) => {
          ...
          const uploadRes = await import_axios.default.put(url, chunkData, {
      ...
      await import_axios.default.post(`${this.apiUrl}/v1/upload/multipart/complete`, {
        key,
        uploadId,
      ...
      if (dualUpload && web3PresignedUrl && web3UploadId) {
        await import_axios.default.put(web3PresignedUrl, bufferToUpload, {
          headers: { "Content-Type": "application/octet-stream" },
          maxBodyLength: Infinity,
          maxContentLength: Infinity
        });
